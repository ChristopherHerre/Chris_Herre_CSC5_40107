...
int main(int argc, char** argv) {
    ...
    //Player *test = new Player[pCount];
    fstream file;
    file.open("bank.bin", ios::in | ios::binary);
    for (short i = 0; i < pCount; i++)
    {
        file.read(reinterpret_cast<char *>(players), sizeof(Player));
    
    }
    file.close();
    for (short i = 0; i < pCount; i++)
    {
        cout << "Player " << i + 1 << " " << players[i].bank << endl;
    }
    delete[] players;
        ...
        short lastPlayer)
{
    ...
    fstream file;
    file.open("bank.bin", ios::out | ios::binary);
    for (short i = 0; i < pCount; i++)
    {
        file.write(reinterpret_cast<char *>(players), sizeof(Player));
    }
    file.close();
